...
Модели OpenAI (только с ключом): `gpt-5.6-sol` — агент и чат; `gpt-4.1-mini` — короткий структурированный
разбор сценария (около 3 секунд); `gpt-4o-mini-transcribe` — распознавание речи.
...
| `OPENAI_EXPLAIN_MODEL` | отдельная быстрая модель для разбора сценария | `gpt-4.1-mini` |
