...
- `pytest` — все проверки зелёные: формула, 7 правил, эталонные числа организаторов;
...
```mermaid
flowchart LR
    U[Управленец] -->|5 решений, вопрос, голос| S[Экран app.py · Streamlit]
    S --> E[Движок akim/]
    subgraph E[Движок akim/]
        V[Проверка правил] --> C[Расчёт Score]
        C --> O[Оптимизатор: улучшить, подбор под цель, место из 694 395]
        C --> X[Разбор компромиссов]
        C --> T[Паспорт района, сравнение, что если, паспорт сценария]
    end
    E --> D[(data/akim.json — данные организаторов)]
    X -.текст, по желанию.-> L[OpenAI]
    L -.-> N{Проверка чисел}
    N --> S
...
- `akim/` — движок: `rules.py` (правила), `scoring.py` (формула), `optimizer.py` (вклад мер, «Улучшить»),
  ...
  (разбор и проверка чисел), `tools.py` (паспорт района, сравнение, «что если»), `brief.py` (паспорт сценария),
  `approvals.py` (утверждённые сценарии), `voice.py` (голос), `llm.py` (подключение модели).
