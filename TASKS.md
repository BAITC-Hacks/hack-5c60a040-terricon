...
## Сейчас, с 16:15 — кто что делает до 18:00

- **Дмитрий** (лимиты Codex кончились в 15:55, его задачи забрал Claude): коммит в часе 17:00–18:00 — в 17:40
  проверка допуска на своей машине по финальной версии из GitHub (`check_scenario.py`, `pytest`), вывод — в `docs/`.
  После 17:55 не отправлять.
- **Codex Виталия:** повторная проверка «только чтение» сделана (16:05): 8 находок закрыты, 2 новые исправлены
  (`41f1f04`); из советов по экрану взяты №3 и №4 (`8963c33`).
- **Claude:** находки README из исследования Дмитрия; проверка названий мер в ответах агента; проверки глазами
  судьи 17:10 · 17:37 (свежий клон, чистый venv, оба способа, Docker); 17:20 — `git rm -r messages`;
  17:30 — финальная отправка; 18:00 — стоп.
- **С 16:00:** форму ответов движка не меняем, только исправления.
- **17:00–17:30 — новый экран `web/` вместо Streamlit (решение Виталия 16:57):** экран `web/index.html` — Codex
  (сессии `01a0cd6c-…`, `01a0ce20-…`), сервер `web/server.py` и `tests/test_web.py` — Claude. Все функции старого
  экрана перенесены и проверены end-to-end в браузере; 100 тестов, сценарий 11/11, Docker — `/api/health`.
...
| Д-5 Docker и запуск без Docker — ✅ Claude (сборка не проверена → Д-9) | В-5 подбор под цель, карта, паспорта, события — ✅ Codex 14:38 |
| Д-6 городские события и устойчивость — ✅ Claude | В-7 понятно за 5 секунд — ✅ Codex 15:15, в истории (`af03cb5`); основной экран — версия Claude ниже |
| Д-7 инструменты Аналитика и Стратега, паспорт сценария, голос — ✅ Claude | В-6 рейтинг команд — не нужен: уже на основном экране |
| Д-8 главный сценарий одной командой — ✅ Дмитрий, 11 из 11 за 2 с | |
| Д-9 чистый клон, оба способа запуска — ✅ Claude 15:23–15:35 (забрал у Дмитрия): pip 1 мин 53 с, Streamlit 1.64 — 5 тестов экрана падали (путь в AppTest), исправлено; Docker: сборка 1 мин, экран и сценарий 11/11 внутри, ключа нет | |
| Д-10 рейтинг команд — ✅ Дмитрий 14:29 | |
| Д-11 живой режим модели: текст разбора и проверка чисел — ✅ Дмитрий 15:07; живой вызов 15:10: поручение идёт от модели без отклонения (56,78, цена 0,46, 6 с), разбор без названий полей, но 18 с | |
| Д-12 разбор моделью быстрее — ✅ начал Дмитрий (`6253c81`…`a103b7d`), доделал Claude: при лимите 350 токенов и `gpt-5-mini` + `none` разбор с ключом всегда уходил в шаблон → лимит 1500 (`b7b8c35`); замер четырёх моделей на пяти запросах, решение команды — `gpt-5.6-sol` везде (`6fc7d02`, раздел README «Почему gpt-5.6-sol»); голос с подсказкой районов (`2f7beed`) | Экран «понять за 5 секунд» с живой картой районов (`app.py` + `city_view.py`) — ✅ Claude, основной с 15:20 по слову Виталия; В-7 Codex — в истории (`af03cb5`), В-6 не нужен: рейтинг уже на экране |
...
## Д-10. Рейтинг команд — ✅ Дмитрий 14:29, по контракту
...
## Д-11. Живой режим модели — ⬜ Дмитрий, после Д-9
...
## В-7. Понятно за 5 секунд — ⬜ Codex, после В-5

Запрос Виталия 14:35: человек, который видит экран впервые (член жюри, аким), за 5 секунд понимает, что
происходит и что делать. Меньше формализации, больше понятных слов. Цвета — государственные цвета Казахстана:
тема уже в `.streamlit/config.toml` (небесно-голубой флага `#00ABC2`, кнопки `#007E99`, золотой `#FEC50C`,
текст `#0B2545`, фон `#F7FBFC`); в CSS брать эти же значения.
ГДЕ: `app.py`, `tests/test_app.py`. Движок не меняется: названия уже есть в `akim.load_data()` —
`indicators[].name`, `measures[].name`, `directions[].name`.
НЕЛЬЗЯ: на экране коды M1…M15 и T1…C2, «D», «ед.», «лаг», «кв.», «Score» без пояснения, английские слова; числа,
которых нет в ответах движка; менять ключи виджетов (`measure_i`, `district_i`, ключи кнопок из тестов).
ГОТОВО:
1. Шапка — полоса небесно-голубого цвета с тонкой золотой линией снизу: «Аким на 5 часов» и строка «Симулятор
   бюджета Астаны: выберите 5 решений на 100 единиц бюджета — движок покажет, как изменится жизнь в каждом районе».
2. Под шапкой, над всеми вкладками — «Главное» одной фразой крупно из полей `evaluate`: «Ваш сценарий поднимает
   индекс качества жизни Астаны с 52,56 до 56,54 (+3,98). Отстающий район — Нура: 52,96 из 100. Острых проблем
   не осталось (было 2). Потрачено 95 из 100.» Недопустимый набор — «Сценарий нельзя принять: <причина из errors>.
   Исправьте — и увидите результат». Ниже три карточки: «Индекс качества жизни (Astana Quality of Life Score)»,
   «Отстающий район», «Острые проблемы — показатели ниже 40 из 100».
3. Числа с запятой, как в текстах движка (56,54), не больше двух знаков, без хвостовых нулей в таблице (38 → 48).
4. Выбор мер: подпись поля «Решение 1» и «Где: район»; в списке — «Школа + детсад (модульное строительство) —
   24 из 100»; под выбранной мерой серой строкой — направление и что улучшает словами: «Соцсфера · школы и детсады
   +16 · заработает через 3 квартала». Лаг — это через сколько кварталов мера начнёт работать (датасет, строка 44).
5. Таблица районов: строки — показатели полными названиями («Школы и детсады», «Качество воздуха»…), столбцы —
   районы, последняя строка — «Индекс района»; легенда словами над таблицей: красный — «острая проблема, ниже 40»,
   жёлтый — «на грани, 40–45», жирный — «изменилось».
6. «Вклад решений» — полосы с названием меры и «+1,45 к индексу», без столбца-номера и без кода меры.
7. «Синергии» → «Решения усиливают друг друга»: «Освещение и камеры + Цифровая платформа обращений — безопасность
   улиц в Нуре +2».
8. Вкладки по шагам: «1. Сценарий» (выбор и результат) · «2. Совет агента» (разбор, поручение, чат) ·
   «3. Сравнить и утвердить» · вкладки В-5.
9. Разбор агента: «Что получилось», «Где риск», «Что будет дальше»; источник — «Разбор составлен по расчётам
   движка» (шаблон) или «Разбор написал ИИ по расчётам движка, числа сверены» (модель).
Тест: на странице нет «M7 ·», «T1», «лаг», « ед.»; «Главное» содержит 56,54 и Нуру; прежние тесты зелёные.

## В-6. Рейтинг команд — ⬜ Codex, после В-7 (движок Д-10 готов)
