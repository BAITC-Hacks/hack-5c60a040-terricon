...
| Д-8 главный сценарий одной командой — 🔨 Дмитрий: 14:18 отправлен, 6 из 6; досдаёт 5 пунктов и тест до 14:40 | |
| Д-9 чистый клон на ноутбуке Дмитрия, оба способа запуска — ⬜ Дмитрий, до 15:15 | |
| Д-10 рейтинг команд — ⬜ Дмитрий, до 15:50 | |
| Д-11 живой режим модели: текст разбора и проверка чисел — ⬜ Дмитрий, после Д-10 | |
...
## Д-11. Живой режим модели — ⬜ Дмитрий, после Д-10

Находки Claude 14:25 — первый настоящий вызов `gpt-5.6-sol` с ключом (все тесты до этого — на подмене):
1. `akim.explain(пример)` → `mode: "llm"`, но 15,9 с, и в тексте названия полей вместо слов: «снял critical_now
   до отсутствия новых критических значений», «даёт score 57.21 и delta 0.67». Ожидали: русский текст для акима без
   названий полей («Score города», «прирост», «показатели ниже 40») и ответ за несколько секунд — например,
   `reasoning_effort="none"`: с ним чат с инструментами ответил за 6,6 с.
2. `akim.chat("Улучши, но не ухудшай воздух в Сарыарке", пример, [])` → ответ модели отклонён проверкой чисел
   («Ответ модели отклонён: числа 48,75 не из расчёта»), ушёл в правила; итог верный — 56.78, цена 0.46. Но 48,75 —
   воздух Сарыарки до мер, настоящее число из данных. Ожидали: числа из ответов инструментов, которые модель
   получила, считаются своими; серой пометки «отклонён» на экране нет.
ГДЕ: `akim/explainer.py`, `akim/agent.py`, их тесты.
НЕЛЬЗЯ: пропускать числа, которых нет ни в расчёте, ни в ответах инструментов; `temperature`.
ГОТОВО: тест на подмене — число из ответа инструмента (48.75) принимается, выдуманное отклоняется; с ключом — строка
Claude: время ответа и пример текста.
