...
## Сейчас, с 15:35 — кто что делает до 18:00

- **Дмитрий:** Д-12 — разбор моделью быстрее (сейчас 16–18 с на каждый новый набор): для `explain`
  `reasoning_effort="none"`, проверить, что текст не хуже, тест. Коммит в каждом часу (16:00–17:00, 17:00–18:00),
  после 17:55 не отправлять. По желанию — ID своей сессии Codex для README.
- **Codex Виталия:** проверка экрана «только чтение» (`app.py`, `city_view.py`, `tests/test_app.py`) — список
  «файл:строка — что не так — чем кончится на показе». Дальше — только исправления по находкам, новых функций нет.
- **Claude:** проверки глазами судьи 16:00 · 17:10 · 17:35 (свежий клон, чистый venv, оба способа, Docker);
  находки Codex и Д-12 — проверить; 17:20 — `git rm -r messages`; 17:30 — финальная отправка; 18:00 — стоп.
...
| Д-12 разбор моделью быстрее (сейчас 18 с на каждый новый набор) — ⬜ Дмитрий | Экран «понять за 5 секунд» с живой картой районов (`app.py` + `city_view.py`) — ✅ Claude, основной с 15:20 по слову Виталия; В-7 Codex — в истории (`af03cb5`), В-6 не нужен: рейтинг уже на экране |
