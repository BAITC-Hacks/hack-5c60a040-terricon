...
| Д-5 Docker и запуск без Docker — ✅ Claude (сборка не проверена → Д-9) | В-5 подбор под цель, карта, паспорта, события — 🔨 Codex |
| Д-6 городские события и устойчивость — ✅ Claude | В-7 понятно за 5 секунд: слова вместо кодов, «Главное», цвета флага — ⬜ после В-5 |
| Д-7 инструменты Аналитика и Стратега, паспорт сценария, голос — ✅ Claude | В-6 рейтинг команд — ⬜ после В-7 |
...
| Д-12 разбор моделью быстрее (сейчас 18 с на каждый новый набор) — ⬜ Дмитрий | Экран v2 `app_v2.py` + `city_view.py` («понять за 5 секунд», живая карта районов) — ✅ Claude 14:56, ждёт решения Виталия: сделать основным |
